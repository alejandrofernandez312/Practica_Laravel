    ...
    <form action="{{ route('tareas.update', $tarea->tarea_id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="container row">
            <div class="col-6">
                ...
                @enderror
                <p>Descripción:
                    <br><input type="text" name="descripcion" class="form-control" id="descripcion"
                ...
                <p>Fecha creación:
                    <br><input type="text" name="f_creacion" class="form-control" id="f_creacion"
                        value="{{ date('d-m-Y', strtotime($tarea->f_crea)) }}" readonly>
                </p>
            ...
            <div class="col-6">
                <p>Fecha realización:
                    <br><input type="date" name="f_realizacion" class="form-control" id="f_realizacion"
                        value="{{ $tarea->obtenerFechaRealizacion() }}">
                </p>
                <p>Anotaciones anteriores:
                    <br><input type="text" name="anot_anteriores" class="form-control" id="anot_anteriores"
                        value="{{ old('anot_anteriores', $tarea->anot_anteriores) }}">
                </p>
                <p>Anotaciones posteriores:
                    <br><input type="text" name="anot_posteriores" class="form-control" id="anot_posteriores"
                        value="{{ old('anot_posteriores', $tarea->anot_posteriores) }}">
                </p>
                <p>Fichero:
                    <br><input type="text" name="fichero" class="form-control" id="fichero"
                    ...
                    <br><select name="cliente" class="form-select" id="cliente">
                        @foreach ($clientes as $cliente)
                            @if ($cliente->cliente_id == $tarea->cliente->cliente_id)
                                <option value="{{ $tarea->cliente->cliente_id }}" selected>{{ $tarea->cliente->nombre }}
                                    ({{ $tarea->cliente->email }})</option>
                            @else
                        ...
                        @foreach ($empleados as $empleado)
                            @if ($tarea->empleado != null)
                                @if ($empleado->empleado_id == $tarea->empleado->empleado_id)
                                    <option value="{{ $tarea->empleado->empleado_id }}" selected>
                                        {{ $tarea->empleado->nombre }} ({{ $tarea->empleado->email }})</option>
                                @else
                                    <option value="{{ $empleado->empleado_id }}">{{ $empleado->nombre }}
                                        ({{ $empleado->email }})</option>
                                @endif
                            @else
                                <option value="{{ $empleado->empleado_id }}">{{ $empleado->nombre }}
                                    ({{ $empleado->email }})</option>
